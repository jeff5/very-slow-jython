...
public class TestEx8 {
    ...
    public static void setUpClass() {
        // Built-in types
        TypeHandler uptoShortHandler = new UptoShortHandler();
        Runtime.registerTypeFor(Byte.class, uptoShortHandler);
        Runtime.registerTypeFor(Short.class, uptoShortHandler);
...
// assertThat(UnaryOpCallSite.fallbackCalls, is(ufb + 2));
    ...
    static class LongHandler extends MixedNumberHandler {
        ...
        private static Object div(Long v, Number w) {
            return v.doubleValue() / w.doubleValue();
        }

        private static Object div(Number v, Long w) {
            return v.doubleValue() / w.doubleValue();
        }
    ...
    /**
     * Operations handler for narrow Java integers up to
     * <code>Short</code>, so as to make each a Python <code>int</code>,
     * returning <code>Integer</code>.
     */
    @SuppressWarnings(value = {"unused"})
    static class UptoShortHandler extends PureNumberHandler {
        ...
        /*
         * Although the following all take Number arguments, they won't be
         * called unless the actual type is representable in a short.
         */
