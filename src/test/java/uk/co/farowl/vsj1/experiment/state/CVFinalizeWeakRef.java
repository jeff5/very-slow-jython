...
public class CVFinalizeWeakRef {
    ...
    public static class MyRuntimeImpl extends AbstractMap<String, Object>
        ...
        public static class MyClassValue
                extends ClassValue<Reference<MyValue>> {

            @Override
            protected Reference<MyValue> computeValue(Class<?> type) {
                MyValue ret = registry.get(type);
                // It would be a mistake to deref to the value:
        ...
        public void run() {
            ...
            globals = new HashMap<>();
            // Work
            Reference<MyValue> w = myValueMap.get(Integer.TYPE);
            globals.put("weak", w);
            globals.put("strong", w.get());
    ...
    static final String OUTERCLASS = CVFinalizeWeakRef.class.getName();
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            Object valueRef = globals.get("weak");
            Object value = globals.get("strong");

            /*
             * Drop references to the runtime instance, but keep the
             * MyValue and the weak Reference to it: what is garbage now? (It
            ...
            /*
             * Drop reference to the MyValue, but keep the weak Reference to it:
             * what is garbage now? (It should be: MyValue and the class
             * loader we used.)
             */
            value = null;
