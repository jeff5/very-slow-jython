...
public abstract class TreePython {
     ...
     * using {@link Node#accept(Visitor)}, returning <code>null</code>.
     */
    public static class AbstractVisitor<T> implements Visitor<T> {

        /** Visit all non-null nodes in some collection. */
        protected T visitAll(java.util.Collection<? extends Node> nodes) {
            for (Node node : nodes) {
                visitIfNotNull(node);
            }
            return null;
