...
class Clinic {
    ...
    static {
        try {
            intArgMH = LOOKUP.findStatic(Clinic.class, "intArg",
                    MethodType.methodType(int.class, O));
            doubleArgMH = LOOKUP.findStatic(Clinic.class, "doubleArg",
     ...
     * {@code PyObject} or a sub-class. Where the existing handle
     ...
     * supplied. Where the existing handle expects a {@code PyObject} or
     * a sub-class, a {@code null} conversion will be supplied. These
     * conversions will throw a Python exception (often
     * {@link TypeError}), according to the usual behaviour of Python,
     * when invoked on objects they cannot convert.
     * <p>
     * The returned array is suitable as an argument to
     * {@code MethodHandles.filterArguments.}
     *
     * @param mt type to adapt.
     * @return array of filter-adaptors to expect {@code PyObject}.
     */
     ...
     * the given type, to one that the returns {@code PyObject} or a
     * sub-class. This adaptor will often be a constructor for the
     * implementation type or equivalent convenience method. If the
     * return type of is {@code void.class}, the adaptor takes no
     * arguments and produces {@link Py#None}. The handle produced is
     * suitable as an argument to
     * {@code MethodHandle.filterReturnValue}.
     * <p>
     * If the return type is already {@code PyObject} or a sub-class,
     * this method returns {@code null} (which is not suitable as an
     * argument to {@code MethodHandle.filterReturnValue}). Client code
     * must test for this.
     *
     * @param mt type to adapt.
    ...
    private static MethodHandle adaptParameterToObject(Class<?> c) {
        if (c.isPrimitive()) {
            ...
            if (c == String.class) {
                ...
                // The method expects a PyObject
                return null;
            } else if (O.isAssignableFrom(c)) {
                // The method expects some sub-class of PyObject
                return null;
            }
        }
     ...
     */
    private static int intArg(PyObject o) throws TypeError, Throwable {
        Class<? extends PyObject> c = o.getClass();
        if (c == PyLong.class) {
     ...
     */
    private static double doubleArg(PyObject o)
     ...
     */
    private static String stringArg(PyObject o)
            throws TypeError, Throwable {
        Class<? extends PyObject> c = o.getClass();
    ...
    private static MethodHandle adaptReturnToObject(Class<?> c) {
        if (c.isPrimitive()) {
            ...
            if (c == String.class) {
                ...
                // The value is already some kind of PyObject
                return null;
            }
        }
        throw new InterpreterError(
