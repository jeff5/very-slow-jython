...
class Slot {
    ...
    static final MethodType TERNARY = MethodType.methodType(O, O, O, O);
    static final MethodType RICHCMP = MethodType.methodType(O, O, O, CMP);
    ...
    static {
        ...
        RICHCMP_EMPTY = MethodHandles.dropArguments(EMPTY, 0, O, O, CMP);
    }
    ...
    enum Signature {
        ...
        Signature(MethodType type, MethodHandle empty) {
            this.type = type;
            this.empty = empty;
        }
        ...
        static Signature matching(MethodType t) { return sigMap.get(t); }
    }
     ...
     * the {@code MappingMethods}, the {@code BufferMethods}, and the slots
     * that are members of the {@code PyType} object itself.
     * <p>
    ...
    private static class EnumUtil {
         ...
         * {@link Any#setSlot(PyType, MethodHandle)}, when a bad handle is
         * presented.
         *
         ...
         */
        static InterpreterError slotTypeError(Any s, MethodHandle mh) {
    ...
    enum SQ implements Any {
        ...
        length("length", Signature.UNARY);

        final String methodName;
        final Signature signature;
        final VarHandle slotHandle;

        SQ(String methodName, Signature signature) {
            this.methodName = methodName;
            this.signature = signature;
            this.slotHandle = EnumUtil.slotHandle(this);
        }
