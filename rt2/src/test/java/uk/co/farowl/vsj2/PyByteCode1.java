...
class PyByteCode1 {
    ...
    private static class CPythonFrame extends PyFrame {
        ...
        /**
         * Create a {@code CPythonFrame}, which is a
         * {@code PyFrame} with the storage and mechanism to execute
         * code. The constructor specifies the back-reference to the
         * current frame (which is {@code null} when this frame is
         * first in the stack) via the {@code ThreadState}. No other argument may be {@code null}.
         *
    ...
    void test_load_store_name_ex1() {
        ...
        frame.eval();
        // a == 'World'
        assertEquals("a", new PyUnicode("World"), globals.get(
            new PyUnicode("a")));
        // b == 2.0
        assertEquals("b", new PyFloat(2.0), globals.get(
            new PyUnicode("b")));
        // c == 'begins!'
        assertEquals("c", new PyUnicode("begins!"), globals.get(
            new PyUnicode("c")));
    }
