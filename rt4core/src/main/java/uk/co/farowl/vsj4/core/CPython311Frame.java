...
class CPython311Frame extends PyFrame<CPython311Code> {
                ...
                switch (opword >> 8) {
                    ...
                    case Opcode311.LOAD_NAME: {
                        ...
                        if (v == null) {
                            ...
                            if (v == null)
                                throw PyErr.format(PyExc.NameError,
                                        NAME_ERROR_MSG, name);
                        }
                        s[sp++] = v; // PUSH
                        break;
    ...
    private static final Object[] EMPTY_OBJECT_ARRAY = Util.EMPTY_ARRAY;
     ...
     * @throws PyBaseException (TypeError) if {@code v} is not iterable
     * @throws PyBaseException (ValueError) if the number of arguments
     *     from the itearable is not as expected.
    ...
    private static int unpackIterable(Object v, int argcnt,
        ...
        if (argcntafter == -1) {
            ...
            if (Abstract.next(it) != null) {
                throw PyErr.format(PyExc.ValueError, TOO_MANY_TO_UNPACK,
                        argcnt);
            }

        } else {
            // List the rest of the iterator (size unknowable).
            PyList star = PySequence.list(it);
            s[--sp] = star;
            count++;
            ...
            if (len < argcntafter) {
                throw PyErr.format(PyExc.ValueError,
                        UNPACK_EXPECTED_AT_LEAST, argcnt + argcntafter,
                        argcnt + len);
            }

            // Pop the "after-star" args off list.
            for (int j = argcntafter; j > 0; j--, count++) {
                s[--sp] = star.get(len - j);
            }
            // Resize the list.
            star.subList(len - argcntafter, len).clear();
        }
        return top;
    }
     ...
     * @throws PyBaseException (AttributeError) if the named attribute
     *     does not exist
    ...
    private void getMethod(Object obj, String name, int offset)
        ...
        BaseType objType = BaseType.of(obj);

        // If type(obj) defines its own __getattribute__ use that.
        if (!objType.hasFeature(KernelTypeFlag.OBJECT_GETATTRIBUTE)) {
         ...
         * access mechanism PyBaseObject.__getattribute__. It differs in
         * that, if the look-up leads to a method descriptor, we avoid
         * binding it into a short-lived bound method object.
         */
        ...
        Object attr = objType.lookup(name);
        if (attr != null) {
            // Found in the type, it might be a descriptor
            Representation attrRep = Abstract.representation(attr);
            descrGet = attrRep.op_get();
            BaseType attrType = attrRep.pythonType(attr);
            if (attrType.hasFeature(TypeFlag.METHOD_DESCR)) {
                /*
                 * We found a method descriptor, but will check the
                 * instance dictionary for a shadowing definition.
                 */
                methFound = true;
            } else if (attrType.isDataDescr()) {
                // typeAttr is a data descriptor so call its __get__.
                try {
                    valuestack[offset] = null;
                    valuestack[offset + 1] =
                            descrGet.invokeExact(attr, obj, objType);
                    return;
        ...
        if (attr != null) {
             ...
             */
            valuestack[offset] = null;
            valuestack[offset + 1] = attr;
            return;
        }

        // All the look-ups and descriptors came to nothing :(
        throw _PyUtil.noAttributeError(obj, name);
    }
     ...
     * @throws PyBaseException (SystemError) if {@code keys} is not the
     *     expected size (or not a {@code tuple})
     */
    private int constKeyMap(int sp, int oparg) throws PyBaseException {
        // Shorthand
        Object[] s = valuestack;
        Object o = s[--sp];
        try {
        ...
        throw PyErr.format(PyExc.SystemError, BAD_BUILD_CONST_KEY_MAP);
    }
     ...
     * @return
     */
    private static PyBaseException noLocals(String action,
            String name) {
        return PyErr.format(PyExc.SystemError,
                "no locals found when %s '%s'", name);
    }

    /**
     * Create an {@link PyExc#UnboundLocalError UnboundLocalError} to
     * throw naming a fast local variable that was "referenced before
     * assignment". The name is obtained from the index in
     * {@link #fastlocals} of the missing variable, by consulting the
     * code object.
     *
     * @param oparg index in {@link #fastlocals} of the variable
     * @return exception to throw
     */
    private PyBaseException unboundFast(int oparg) {
    ...
    /**
     * Create a {@link PyExc#NameError NameError} (or specific sub-class
     * {@link PyExc#UnboundLocalError UnboundLocalError} to throw naming
     * a cell variable that was "referenced before assignment". The name
     * is obtained from the index in {@link #freevars} of the empty
     * cell, by consulting the code object.
     *
    ...
    /**
     * Create a {@link PyExc#TypeError TypeError} to throw when keyword
     * arguments appear not to be a mapping. {@code dict.merge} raises
     * {@link PyExc#AttributeError AttributeError} (percolated from an
     * attempt to get 'keys' attribute) if its second argument is not a
     * mapping, which we convert to a {@link PyExc#TypeError TypeError}.
     *
    ...
    private static PyBaseException kwargsTypeError(Object func,
            Object kwargs) {
        String funcstr = _PyUtil.functionStr(func);
        return Abstract.argumentTypeError(funcstr, "**", "a mapping",
                kwargs);
    }

    /**
     * Create a {@link PyExc#TypeError TypeError} to throw when a
     * duplicate key turns up while merging keyword arguments to a
     * function call.
     *
     ...
     */
    // Compare CPython format_kwargs_error in ceval.c
    private static PyBaseException kwargsKeyError(PyKeyError ke,
            Object func) {
        /*
         * PyDict.merge raises KeyError (percolated from an attempt to
         * assign an existing key), which we convert to a TypeError.
         */
        String funcstr = _PyUtil.functionStr(func);
