 ...
 */
public class PySlice implements WithClass {

    /** The type of Python object this class implements. */
    static final PyType TYPE = PyType.fromSpec( //
            new TypeSpec("slice", MethodHandles.lookup())
    ...
    static PyObject slice_new(PyNewWrapper new_, boolean init, PyType subtype, PyObject[] args,
                              String[] keywords) {
        if (args.length == 0) {
            throw PyErr.format(PyExc.TypeError, "slice expected at least 1 arguments, got " + args.length);
        } else if (args.length > 3) {
            throw PyErr.format(PyExc.TypeError, "slice expected at most 3 arguments, got " + args.length);
        }
    ...
    */
     ...
     * @throws PyBaseException (TypeError) if any index has no
     *     {@code __index__}
     * @throws PyBaseException (ValueError) if {@code this.step==0}
     * @throws Throwable from implementation of {@code __index__}
     */
    // Compare CPython PySlice_GetIndicesEx in sliceobject.c
    Indices getIndices(int length) throws PyBaseException, Throwable {
    ...
    public class Indices {
        ...
        /**
         * Absolute index in the source sequence of the first element to
         * be taken by the slice. If {@link #slicelength}{@code  != 0},
         * this index lies within the bounds of the sequence.
         */
        public final int start;
        /**
         * Absolute index relative to the source sequence that is the
         * image of {@link PySlice#stop}. Dealing correctly with a step
         * size other than one is difficult. Clients should normally
         * choose {@link #slicelength}, to decide how many elements to
         * take from the sequence, rather than use {@code stop} to
         * decide when to stop.
         */
        public final int stop;
        /**
         * The index step to make when selecting elements from the
         * source sequence. Never zero.
         */
        public final int step;
        /**
         * The number of elements to select from the source sequence,
         * and therefore the length of the slice to be generated.
         */
        public final int slicelength;

        /**
         * Extract the {@code start}, {@code stop} and {@code step} data
         * members from the parent {@code slice} as Java {@code int}s in
         * an instance of {@code Indices}, then adjust {@code start} and
         * {@code stop} assuming they apply to a sequence of the
         * specified {@code length}. Store in {@code slicelength} the
         * number of elements the parent slice will take from that
         * sequence.
         * <p>
         * Out of bounds indices are clipped in a manner consistent with
         * the handling of normal slices. The idiom:<pre>
         ...
         * <p>
         * <b>Detail:</b> Before adjusting to the specific sequence
         * length, the following occurs. Extract the {@code start},
         * {@code stop} and {@code step} data members from the parent
         * {@code slice} and convert them to Java {@code int}s using
         * their {@code __index__} methods, or mapping {@code None} to
         * conventional values. Silently reduce values larger than
         * {@code Integer.MAX_VALUE} to {@code
         Integer.MAX_VALUE}. Silently boost {@code start} and
         * {@code stop} values less than {@code Integer.MIN_VALUE} to
         * {@code
         Integer.MIN_VALUE}. And silently boost {@code step} values less
         * than {@code -Integer.MAX_VALUE} to
         * {@code -Integer.MAX_VALUE}.
         *
         * @param length of the target sequence
         * @throws PyBaseException (TypeError) if any index not
         *     {@code None} has no {@code __index__}
         * @throws PyBaseException (ValueError) if {@code step==0}
         * @throws Throwable from the implementation of
         *     {@code __index__}
         */
        // Compare CPython PySlice_Unpack in sliceobject.c
        // Compare CPython PySlice_AdjustIndices in sliceobject.c
        public Indices(int length) throws PyBaseException, Throwable {
            ...
            /*
             * Here step0 might be MIN = -MAX-1; in this case we replace
             * it with -MAX. This doesn't affect the semantics, and it
             * guards against later undefined behaviour resulting from
             * code that does "step = -step" as part of a slice
             * reversal.
             */
