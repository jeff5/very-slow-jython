 ...
 * {@code List<Object>}. Operations in both APIs are synchronised for
 * safety against concurrent threads.
 * <p>
 * It is {@code synchronized} so that competing threads should be able
 * to access it with roughly the same protection against concurrent
 * modification that CPython offers. There are also necessary safeguards
 * during {@code sort()} to detect modification from within the current
 * thread as a side effect of comparison. Java brings its own safeguard
 * within iterators against structural concurrent modification.
 *
...
public class PyList implements List<Object>, WithClass {
     ...
     */
    public static PyList of(Object... elements) {
        return new PyList(List.of(elements));
    }
    ...
    */
    ...
    synchronized Object __imul__(Object o) {
        ...
        int count = o.asIndex(PyExc.OverflowError);
        ...
        if (size > Integer.MAX_VALUE / count) {
            throw PyErr.format(PyExc.MemoryError, "");
        }
    ...
    */
    ...
    /**
     * Return smallest index where an element in the list Python-equals
     * the argument.
     *
     ...
     * @throws PyBaseException (TypeError) from bad {@code start} and
     *     {@code stop} types
    ...
    final synchronized void list_remove(Object v) throws Throwable {
        ...
        if (i >= 0) {
            ...
            throw PyErr.format(PyExc.ValueError,
                    "%s.remove(x): x not in list", getType().getName());
        }
    }
    ...
    private int find(Object v) throws Throwable {
        int n = list.size();
        for (int i = 0; i < n; i++) {
            if (Abstract.richCompareBool(v, list.get(i),
                    Comparison.EQ)) {
                return i;
            }
        }
    ...
    /**
     * Reverses the items of s in place. The reverse() methods modify
     * the list in place for economy of space when reversing a large
    ...
    private synchronized void sortOnValue(boolean reverse)
            throws Throwable {

        // We shall sort on values using this (Python) comparator
        final ListElementComparator<Object> cmp =
                new ListElementComparator<Object>(reverse) {

                    @Override
                    boolean lessThan(Object o1, Object o2)
                            throws Throwable {
                        return Abstract.richCompareBool(o1, o2,
                                Comparison.LT);
                    }
    ...
    private synchronized void
        ...
        final ListElementComparator<KV> cmp =
                new ListElementComparator<KV>(reverse) {

                    @Override
                    boolean lessThan(KV o1, KV o2) throws Throwable {
                        return Abstract.richCompareBool(o1.key, o2.key,
                                Comparison.LT);
                    }
    ...
    public synchronized boolean containsAll(Collection<?> c) {
        try {
            for (Object o : c) {
                // Use the Python definition of equality (which may
                // throw)
    ...
    public synchronized boolean removeAll(Collection<?> c) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (Object o : c) {
                ...
                // throw)
                try {
                    if (Abstract.richCompareBool(o, item,
                            Comparison.EQ)) {
                        erasures.add(i);
                        break;
                    }
    ...
    public synchronized List<Object> subList(int fromIndex,
            int toIndex) {
        /*
         * XXX There is a difficulty here in our management of
         * concurrency. The sub-list is an *unsynchronised* view on this
         * PyList's private list variable, so it bypasses the
         * synchronisation in the PyList wrapper. Here, and in Jython 2,
         * we wrap this view in a new PyList. Although that is
         * synchronised (on itself), it is not synchronised on this
         * PyList and concurrent access is possible through the two
         * objects. Compare
    ...
    public synchronized <T> T[] toArray(T[] a) {
        ...
        if (Object.class.equals(type)) {
            ...
            for (int i = 0; i < size; i++) {
                a[i] = Abstract.tojava(list.get(i), type);
            }
        }
        return a;
    }
    ...
    public boolean equals(Object other) {
        ...
        synchronized (this) {
            if (other instanceof List) {
                ...
                try {
                    return Abstract.richCompareBool(other, this,
                            Comparison.EQ);
                } catch (Throwable t) {
                    return false;
                }
     ...
     *     {@code KV}.
     */
    private abstract class ListElementComparator<T>
         ...
         */
        ListElementComparator(boolean reverse) {
            this.less = reverse ? 1 : -1;
        }
        ...
        public int compare(T o1, T o2) {
            try {
                ...
                return lessThan(o1, o2) ? less
                        : (lessThan(o2, o1) ? -less : 0);
            } catch (Throwable pye) {
                // Stash the exception to rethrow later
                comparatorException = pye;
                // XXX made up answer may violate contract of compare.
                // Why stash?
                return 0;
            } finally {
    ...
    private boolean erase(List<Integer> erasures) {
        ...
        /*
         * p, q are pointers into the list. We copy elements from
         * list[q] to list[p], for p<q. r is the position of the next
         * erasure. Whenever q reaches an erasure, we advance q over it
         * without copy. Up to the first erasure, no copies are
         * necessary, so we start like this, at he first erasure:
         */
    ...
    private class ListDelegate
        ...
        public void setSlice(PySlice.Indices slice, Object value)
                throws Throwable {
            /*
             * Accept iterables (and iterators) by creating a Java List
             * Jython 2 would also try __tojava__. Necessary?
             */
            ...
            if (D != 0) {
                if (step == 1) {
                    ...
                    if (D < 0) {
                        ...
                        // Copy N elements
                        Collections.copy(list.subList(start, start + N),
                                v);
                    } else {
                        // v is longer by D than slice it replaces.
                        // Copy M elements to [start:stop-D]
                        Collections.copy(list.subList(start, start + M),
                                v.subList(0, M));
                        // Insert the last D elements of v at the slice
                        // stop.
                        list.addAll(stop, v.subList(N - D, N));
                    }
                } else {
                    // Extended slice, but not the same size as the
                    // value
                    throw PyErr.format(PyExc.ValueError,
        ...
        @Override
        public void delItem(int i) throws Throwable { list.remove(i); }
        ...
        public void delSlice(PySlice.Indices slice) throws Throwable {
            ...
            if (M > 0) {
                final int step = slice.step;
                /*
                 * We will step through the list removing items. If we
                 * are stepping backwards, the increments will be
                 * slice.step (negative), but if stepping forwards,
                 * slice.step-1, because the items to right will have
                 * moved one closer, due to the deletion. (There are
                 * faster ways, but how often will it matter?)
                 */
        ...
        /**
         * Implementation of the {@code __contains__} method of
         * sequences. Determine whether the sequence contains an element
        ...
        public boolean __contains__(Object v) throws Throwable {
            ...
            for (Object item : this) {
                if (Abstract.richCompareBool(v, item, Comparison.EQ)) {
                    return true;
                }
            }
            return false;
         ...
         * Determine the number of times the sequence contains an
         ...
         * @throws Throwable from other conversion errors
         */
        public void insert(Object index, Object v)
                throws PyBaseException, Throwable {
            list.add(boundedIndex(index), v);
        }

        /**
         * Accept an object index, treating negative values as
         * end-relative, and bound it to the sequence range. The index
         * object must be convertible by
         * {@link PyNumber#asSize(Object, java.util.function.Function)
         * PyNumber.asSize}. It is not an error for the index value to
         * fall outside the valid range. (It is simply clipped to the
         * nearer end.)
         *
         ...
         * @throws Throwable from other conversion errors
         */
        protected int boundedIndex(Object index)
                throws PyBaseException, Throwable {

            // Convert the argument (or raise a TypeError)
            int i, L = length();
            if (PyNumber.indexCheck(index)) {
                i = PyNumber.asSize(index,
                        s -> PyErr.format(PyExc.IndexError, s));
            } else {
        ...
        @Override
        public int
                compareTo(PySequence.Delegate<Object, PyList> other) {
            try {
                ...
                /*
                 * The lists matched over the length of
                 * Pylist.this.list. The other is the winner if it still
                 * has elements. Otherwise it's a tie.
                 */
                return i < M ? -1 : 0;
                 ...
                 * throws anything.
                 */
                // XXX perhaps need a PyBaseException to wrap Java
                // Throwable
         ...
         * {@code list} for equality. We do this separately from
         ...
         */
        private boolean
                compareEQ(PySequence.Delegate<Object, PyList> other) {
            try {
        ...
        /**
         * Compare this delegate with the delegate of the other
         * {@code list}, or return {@code NotImplemented} if the other
         * is not a {@code list}.
         *
