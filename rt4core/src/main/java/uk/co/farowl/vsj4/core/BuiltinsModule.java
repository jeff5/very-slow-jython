...
class BuiltinsModule extends JavaModule {
    ...
    void exec() {
        ...
        /*
         * We expose these objects by simply adding them to the module
         * dictionary. Methods exposed by the module are exposed by
         * annotating them. The ModuleExposer will enter them in the
         * same dictionary but by a substantially more complex process.
         */
    ...
    static void exec(Object source, @Default("None") Object globals,
        ...
        if (globals == Py.None || globals == null) {
            ...
            // TODO Avoid the cast by passing class (and deducing type)
            globalsDict = (PyDict)PyUtil.typeChecked(globals,
        ...
        if (PyCode.TYPE.check(source)) {
            ...
            int nfree = code.co_freevars().size();
            if (nfree == 0) {
                if (free != null) {
                ...
                if (free == null || free.length != nfree) {
                    throw PyErr.format(PyExc.TypeError, CLOSURE_LENGTH,
                            nfree);
                }
            }
    ...
    /**
     * Create a {@link PyExc#TypeError TypeError} with a message along
     * the lines "exec() closure argument must be T, not X", involving
     * an expected type description T and the type X of {@code o}.
     *
     * @param description of the expected kind of argument
     * @param o actual argument (not its type)
     * @return exception to throw
     */
    private static PyBaseException closureError(String description,
            Object o) {
     ...
     * @throws PyBaseException (SystemError) if there is no current
     *     scope.
     */
    @PythonStaticMethod
    @DocString("Return the dictionary containing the current scope's global variables.")
    static Object globals() throws PyBaseException {
    ...
    static Object locals() throws PyBaseException {
    ...
    private static Object minmax(Object arg1, PyTuple args, Object key,
        ...
        if (n > 0) {
             ...
             */
            result = key == Py.None ? arg1 : Callables.call(key, arg1);
            others = args.iterator();
            if (dflt != Py.None) {
                String name = op == Comparison.LT ? "min" : "max";
                throw PyErr.format(PyExc.TypeError,
                        DEFAULT_WITHOUT_ITERABLE, name);
            }

        } else {
            // Single iterable argument of the values to compare
            result = null;
            // TODO define PySequence.iterable like PyMapping.map?
        ...
        while (others.hasNext()) {
            ...
            if (key != Py.None) { item = Callables.call(key, item); }
            if (result == null) {
                result = item;
