...
package uk.co.farowl.vsj4.modules;
...
import uk.co.farowl.vsj4.core.PySequence.OfInt;
...
public class MarshalModule extends JavaModule {
     ...
     * @throws PyBaseException (ValueError) when an object being read is
     *     over-size or contains values out of range.
     * @throws PyBaseException (TypeError) when file reading returns
     *     non-byte data or a container contains a null element.
    ...
    public PyBytes dumps(Object value, @Default("4") int version)
        ...
        writer.writeObject(value);
        logger.atDebug().setMessage("marshal.dumps() {} bytes")
                .addArgument(bb.length()).log();
     ...
     * @throws PyBaseException (ValueError) when an object being read is
     *     over-size or contains values out of range.
     * @throws PyBaseException (TypeError) when a container contains a
     *     null element.
     * @throws PyBaseException (EOFError) when a partial object is read
     */
    @PythonMethod
    ...
    abstract static class Writer {
         ...
         */
        protected static PyBaseException
                unmarshallableObject(Object v) {
            String t = v == null ? "<null>"
                    : "of type '" + PyType.of(v).getName() + "'";
            return PyErr.format(PyExc.ValueError,
                    "unmarshallable object %s", t);
        }
    }
    ...
    static class StreamWriter extends Writer {
        ...
        void writeByte(int b) {
            try {
                ...
                throw PyErr.format(PyExc.OSError, ioe);
            }
        }
    ...
    public abstract class Reader {
        ...
        public Object readObject() {
            ...
            if (decoder != null) {
                logger.atTrace().setMessage("decoding {}")
                        .addArgument(tc).log();
        ...
        BigInteger readBigInteger() throws PyBaseException {
        ...
        /**
         * Prepare a Python {@link PyBaseException} for throwing, based
         * on the Java {@code IOException}. We may return a Python
         * {@link PyExc#EOFError EOFError} or {@link PyExc#OSError
         * OSError}.
         *
         * @param ioe to convert
         * @return the chosen Python exception
         */
        protected PyBaseException pyException(IOException ioe) {
            if (ioe instanceof EOFException) {
                return endOfData();
            } else {
                return PyErr.format(PyExc.OSError, ioe);
            }
        }

        /**
         * Prepare a Python {@link PyExc#EOFError EOFError} for
         * throwing, with the message that the data are too short. We
         * throw one of these on encountering and end of file or buffer
         * where more of the object was expected.
         *
         * @return a Python exception to throw
         */
        protected static PyBaseException endOfData() {
            return PyErr.format(PyExc.EOFError,
                    "marshal data too short");
        }

        /**
         * Create a {@link PyExc#ValueError ValueError} to throw, with a
         * message along the lines "bad marshal data (REASON(args))"
         *
         * @param reason to insert
         * @param args arguments to fill format
         * @return to throw
         */
        protected static PyBaseException badData(String reason,
                Object... args) {
        ...
        /**
         * Create a {@link PyExc#ValueError ValueError} to throw, with a
         * message along the lines "bad marshal data (REASON)"
         *
         * @param reason to insert
         * @return to throw
         */
        protected static PyBaseException badData(String reason) {
            return PyErr.format(PyExc.ValueError,
                    "bad marshal data (%s)", reason);
        }

        /**
         * Create a {@link PyExc#TypeError TypeError} to throw, with a
         * message along the lines "null object in marshal data for
         * (TYPE)"
         *
         * @param type to insert
         * @return to throw
         */
        protected static PyBaseException nullObject(String type) {
            return PyErr.format(PyExc.TypeError,
                    "null object in marshal data for %s", type);
        }
    }
