...
public class TypeSpec extends NamedSpec {
     ...
     * operations, for example {@code __sub__(MyObject, Integer)}. Such
     * signatures are used in call sites.
     * <p>
     * Types may ignore this technique if the designer is content with a
     * {@code __sub__(MyObject, Object)} that coerces its right-hand
     * argument on each call. (This signature is required by the Python
     * data model and may be defined in the
     * {@link #canonicalClass(Class) canonical base} class, or
     * {@link #methodImpls(Class...) methodImpls}).
     * <p>
     * A binary class-specific must always succeed, and not return
     * {@code Py.NotImplemented}. Rather, the absence of a definition
     * should indicate that the operation is not defined for a given
     * pair of classes. Certain built-ins use the technique to speed up
     ...
     * @return {@code this}
     */
    protected TypeSpec binopImpl(Class<?> binopClass) {
        checkNotFrozen();
        if (this.binopClass != null) {
            throw repeatError("binary operations class");
        }
     ...
     * type. {@code null} if there isn't one, which is the usual case.
     * Certain built-ins use the technique to speed up call sites in JVM
     * byte code compiled from Python.
     *
     * @return class defining binary class-specific operations (or
     *     {@code null})
     */
    public Class<?> getBinopClass() { return binopClass; }
