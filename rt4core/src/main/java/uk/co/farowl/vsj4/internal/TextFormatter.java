 ...
 * In a limited way, it acts like a StringBuilder to which text,
...
public abstract class TextFormatter
     ...
     */
    public TextFormatter(StringBuilder result, FormatSpec spec) {
        super(result, spec);
    }
     ...
     * @param spec parsed conversion specification
     */
    public TextFormatter(FormatSpec spec) {
    ...
    public TextFormatter format(String value) {
        ...
        if (FormatSpec.specified(p) && p < n) {
            /*
             * Estimate the space for the converted result (preempt
             * multiple re-allocation)
             */
            int space = Math.max(spec.width, p);
            result.ensureCapacity(
                    result.length() + space + (bytes ? 0 : space / 4));
            /*
             * A precision p was specified less than the length: we may
             * have to truncate. Note we compared p with the UTF-16
             * length, even though it is the code point length that
             * matters. But the code point length cannot be greater than
             * n.
             */
            int count = 0;
            while (count < p) {
                ...
                result.append(c);
                /*
                 * A high-surrogate will always be followed by a low, so
                 * doesn't count.
                 */
                if (Character.isHighSurrogate(c) && p < n) {
                    /*
                     * Accomplish "not counting" by bumping the limit p,
            ...
            /*
             * We definitely don't need to truncate. Append the whole
             * string.
             */
            lenWhole = n;
            result.append(value);
     ...
     * don't produce them.
     */
    // TODO Check supplementary characters produced by int c-format
    @Override
