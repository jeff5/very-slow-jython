"""
This module provides classes that can be used to describe Java classes,
then to generate the corresponding Java source code. We only implement
a small subset of the possible types.  For instance, we do not support
'protected' class members.

You can find an example of use at the end of this file.  The primary
use case is probably to have one or several instances of `JavaClass`,
where you may add `JavaVariable`s as fields and `JavaMethod`s.
`JavaVariable`s are also used to represent parameters of methods.

Additionally, there is a `JavaInterface` class and some more
specialised variants such as `JavaPseudoEnum` to create a
JavaClass that behaves almost like a Java-enum.  We also support
memoized methods in two variants: a more general version and a
specialised version used in our generated parser.

Although this module was written for the Java-pegen project, it is
fairly general and is probably adaptable to other projects.

Much of this code originated in Tobias Kohn's Java-pegen project. It
has been mildly adapted for use in Jython.
"""
...
# Commonly used names we choose to avoid
_JAVA_UBIQUITOUS_NAMES = [
    'String', 'List', 'Set', 'Map',
]
...
_JAVA_KEYWORDS = {
    ...
    'catch', 'extends', 'try',
    'final', 'interface','void',
    'class', 'finally',
    'const', 'native', 'super', 'while', '_',
    # Modifiers missing from _JAVA_MODIFIERS
    'transient', 'volatile',
}
...
_JAVA_RESERVED_NAMES = set(_JAVA_KEYWORDS) | set(_JAVA_PRIMITIVES.keys()) | \
    set(_JAVA_MODIFIERS) | set(_JAVA_UBIQUITOUS_NAMES)


def default_value_for_java_type(j_type: str) -> str:
    """
...
def checked_java_name(name: str) -> str:
    """Return the name argument if a valid Java name.
    
    If not, we shall raise a ValueError (or TypeError).
    """
    if not isinstance(name, str):
        raise TypeError("name must be a string")
    allowed = re.compile(r"[a-zA-Z_$][a-zA-Z_0-9$]*")
    if not (allowed.fullmatch(name)):
        raise ValueError(f"A Java name contains only letters, digits and _ ('{name}')")
    if name in _JAVA_RESERVED_NAMES:
        raise ValueError(f"A Java name cannot be a keyword or very common type ('{name}')")
    return name
...
def to_java_name(name: str, *, prefix=None,
    ...
    Options allow the caller to specify a prefix and a suffix on the
    name. `start_with_upper`, if given and not None, determines the
    case of the first letter of the result (after the optional prefix).
    Letters following '_' in the original are made uppercase.

    If the final result would be a reserved word (Java keywords and
    some others), an underscore `_` will be appended.
    """
    def add(s:str, m:Callable[[str], str]):
            """Apply m to first letter of s and add to segments"""
            r = m(c:=s[0])
            if r == c:
                # Add s unchanged
                segments.append(s)
            else:
                # Add initial letter transformed and rest of s (if any)
                segments.append(r)
                if len(s) > 1:
                    segments.append(s[1:])

    segments = []  # parts to concatenate finally
    ...
    for s in name.split('_'):
        if s:
            if first_segment and start_with_upper is not None:
                # start_with_upper decides case of first letter
                add(s, str.upper if start_with_upper else str.lower)
            else:
                # everywhere else we capitalise
                add(s, str.upper)
        first_segment = False
    ...
    # Concatenate and keyword check
    if (java_name := ''.join(segments)) in _JAVA_RESERVED_NAMES:
        java_name += '_'
    return java_name


class JavaDeclarable:
    """
    Any (supported) entity that would need declaration.

    This is a base class that contains the properties, checks and
    accessors common to classes, members, local variables, etc. for
    which we need to generate code.
    """
    def __init__(
            self,
            java_name: str,
        ...
        self.name = checked_java_name(java_name)
        self.type = java_type if java_type else 'Object'
...
class JavaMember(JavaDeclarable):
    """
    A member of a class, meaning a field, method or nested class.
    Members have attributes relating to visibility and staticness.
    """
    def __init__(
            self,
            java_name: str,
        ...
        super().__init__(java_name, java_type, comment=comment,
                         is_final=is_final)
    ...
    def create_optional_clone(self) -> 'JavaVariable':
        ...
        """
        var = JavaVariable(self.name, self.type, self.init_value, is_optional=True)
...
class JavaField(JavaMember):
    """
    A field member of a class. Fields are members with a value
    that must be given initially if they are final.
    """
    def __init__(
            self,
            java_name: str,
        ...
        super().__init__(java_name, java_type, comment=comment,
                         is_public=is_public, is_private=is_private,
                         is_static=is_static, is_final=is_final)
        self.init_value = init_value
        # TODO consider managing one layer up since not a Java concept
        # This reflects use of ? in the ASDL. Type must be a reference (I think).
        self.is_optional = is_optional

    def __str__(self):
        if self.init_value:
            return f"{self.type} {self.name} = {self.init_value}"
        else:
    ...
    def forced_init_value(self):
        if not self.init_value:
            return default_value_for_java_type(self.type)
        else:
    ...
    def interface_declarations(self) -> Optional[str]:
        ...
        if not self.is_static and not self.is_private:
            decls = f"  {self.type} get{self.title_name}();\n"
            if not self.is_readonly:
                # TODO Shouldn't this be a JavaMethod?
                decls += f"\n  void set{self.title_name}({self.type} {self.name});\n"
            return decls
        else:
            return None
    ...
    def getter(self) -> str:
        ...
        # TODO Shouldn't this be a JavaMethod?
        s = ' static' if self.is_static else ''
        return f"  public{s} {self.type} get{self.title_name}() {{\n    return this.{self.name};\n  }}"
    ...
    def is_optional(self) -> bool:
        ...
        """
        # TODO Surely related to self.is_optional, but isn't.
    ...
    def is_optional(self, value: bool):
        ...
        if value:
            if self.init_value is None:
                self.init_value = default_value_for_java_type(self.type)
        else:
    ...
    def setter(self) -> str:
        ...
        s = ' static' if self.is_static else ''
        n = self.name
        return f"  public{s} void set{self.title_name}({self.type} {n}) {{\n    this.{n} = {n};\n  }}"
...
class JavaVariable(JavaDeclarable):
    """
    A Java variable is a local variable or a method parameter.
    """
    ...
    def __init__(
        ...
        super().__init__(java_name, java_type, comment=comment,
                         is_final=is_final)
        self.init_value = init_value
        # TODO remove this by unconfusing ASDL Constructor and JavaMethod
        # We use this to decide how many Java constructors to generate.
        self.is_optional = False

    @staticmethod
    def fromField(field:JavaField) -> "JavaVariable":
        var = JavaVariable(field.name, field.type)
    ...
    def declaration(self) -> str:
        ...
        """
        f = ' final' if self.is_final else ''
        return f"{self.get_comment()}  {f} {str(self)};"
...
class JavaMethod(JavaMember):
    """
    Represents a Java method (or constructor via the subclass JavaConstructor).
    """
    ...
    def __init__(
        ...
        super().__init__(java_name, java_return_type, comment=comment,
                         is_public=is_public, is_private=is_private,
                         is_static=is_static, is_final=is_final)
        self.cls = cls
        ...
        if self.is_constructor and java_return_type is None:
            self.java_return_type = java_name
        else:
            self.java_return_type = java_return_type if java_return_type is not None else 'void'
        ...
        if is_override is None:
            is_override = java_name in _JAVA_DEFAULT_METHODS
        self.is_override = is_override

    def __repr__(self):
        return f"{self.__class__.__name__}({self.cls!r}, {self.name!r}, {self.java_return_type!r})"
    ...
    def _get_body(self) -> str:
        if isinstance(self.body, (list, tuple)):
            ...
            if self.cls.has_base_class:
                ...
                super_args = ', '.join(a.name for a in base_fields)
                code.append(f"super({super_args});")
            for a in self.args:
                code.append(f"this.{a.name} = {a.name};")
            return '    ' + '\n    '.join(code)
    ...
    def _get_head(self) -> str:
        ...
        if self.is_constructor:
            ...
            return f"{p}{s}{generics}{self.java_return_type} {self.name}({self._get_args()})"

    def _get_iface_head(self, n: int) -> str:
        generics = f"<{','.join(self.generics)}> " if self.generics else ''
        return f"{generics}{self.java_return_type} {self.name}({self._get_args(n)});"
    ...
    def get_all_declarations(self) -> str:
        ...
        are created.
        """
        # Count the number of optional arguments (fields declared with ? in ADSL)
        n_opt = sum(a.is_optional for a in self.all_args)
        # We create a supplementary constructor for each
        if n_opt > 0:
            p = 'public ' if self.is_public else 'private'
            s = 'static ' if self.is_static else ''
            if self.is_constructor:
                ...
                generics = f"<{','.join(self.generics)}> " if self.generics else ''
                head = f"{o}{p}{s}{generics}{self.java_return_type} {self.name}"
                ret = ('return ' if self.java_return_type != 'void' else '') + self.name
            decls = [
                self.declaration
            ]
            ...
            for i in range(n_opt):
                ...
                for a in self.all_args:
                    if a.is_optional:
                        j -= 1
                        if j < 0:
                            args.append(default_value_for_java_type(a.type))
                            continue
...
class JavaConstructor(JavaMethod):
    ...
    In support of the ADSL constructor argument syntax "?", meaning
    the field is optional, we support the generation in
    get_all_declarations() of multiple constructors omitting
    successively more of the optional arguments. This does not create
    all the possibilities you might find useful, although they are
    necessarily unique.
    """
    ...
    def __init__(
        ...
        super().__init__(cls, java_name, java_name, args, body,
                         comment=comment, generics=generics,
                         is_public=is_public, is_private=is_private,
                         is_override=False)
    ...
    def all_args(self) -> list[JavaVariable]:
        """
        Returns all the arguments of this constructor definition, and
        those for the base class.
        """
        # TODO the logic seems ASDL-specific so belongs up a layer
        if self.cls.has_base_class:
            return self.args + [f for f in self.cls.base_fields]
        else:
            return self.args
    ...
    def declaration(self) -> str:
        ...
        Returns the full declaration of the function or method.
        """
        return f"{self.get_comment()} {self._get_head()} {{\n{self._get_body()}\n  }}"

    def get_all_declarations(self) -> str:
        """
        If some arguments are optional, this will create a number of
        differently long method declarations/implementations.
        Otherwise, the result will be the same as from `declaration`.

        Note: this method is not a property to make it more explicit
        that a number of different method declarations are created.
        """
        # Count the number of optional arguments (fields declared with ? in ADSL)
        n_opt = sum(a.is_optional for a in self.all_args)
        # We create a supplementary constructor for each
...
class JavaMemoizedMethod(JavaMethod):
    ...
    def __init__(
        ...
        super().__init__(cls, name, java_return_type, args, body, comment=comment,
    ...
    def _get_head_private(self) -> str:
        s = 'static ' if self.is_static else ''
        ret_type = self.java_return_type
    ...
    def declaration(self) -> str:
        ...
        if self.hash_j_type is None and self.args:
            hash_j_type = self.args[0].type
        else:
            hash_j_type = self.hash_j_type
        ret_type = _JAVA_PRIMITIVES.get(self.java_return_type, self.java_return_type)
...
class JavaClass:
    ...
    def __init__(
        ...
        self.constructor = JavaConstructor(self, name)
        if isinstance(generics, str):
            generics = generics.split(',')
    ...
    def add_field(self, name: str, j_type: str) -> JavaField:
        var = JavaField(name, j_type)
        self.add(var)
        return var
    ...
    @property
    def all_fields(self) -> list[JavaField]:
        """
    ...
    @property
    def base_fields(self) -> list[JavaField]:
        """
...
class JavaPseudoEnum(JavaClass):
    ...
    def __init__(self, name: str, base_cls: str|JavaClass|None = None, package: Optional[str] = None, **kwds):
        ...
        self.add(JavaField('name', 'String', is_final=True, is_private=True))
