 ...
 * interface.
 */
public interface PyObjectDict extends CraftedType {

    /**
     * The dictionary of the instance, (not necessarily a Python
     * {@code dict} or writable. If the returned {@code Map} is not
     * writable, it should throw a Java
     * {@code UnsupportedOperationException} on attempts to modify it.
     *
     * @implSpec A class that implements {@code PyObjectDict} should
     *     always return a mapping, which may be
     *     {@code Collections.emptyMap()} if the instance dictionary is
     *     intended to be permanently empty.
