...
//@formatter:on
    ...
    /**
     * Specification in which we assemble information about a method in
     * advance of creating a method definition or method descriptor.
     */
    static class MethodSpec extends CallableSpec {
        ...
        PyMethodDescr asAttribute(PyType objclass, Lookup lookup) {
            if (objclass.acceptedCount == 1)
                return createDescrForInstanceMethodSingle(objclass,
                        lookup);
            else
                return createDescrForInstanceMethodMultiple(objclass,
                        lookup);
        }
         ...
         * @throws InterpreterError if the method type is not supported
         */
        private PyMethodDescr createDescrForInstanceMethodSingle(
         ...
         * @throws InterpreterError if the method type is not supported
         */
        private PyMethodDescr createDescrForInstanceMethodMultiple(
                PyType objclass, Lookup lookup)
                throws InterpreterError {
    ...
    /**
     * Specification in which we assemble information about a method in
     * advance of creating a special method descriptor.
     */
    static class WrapperSpec extends BaseMethodSpec {

        private WrapperSpecification wrapperSpec;

        WrapperSpec(Slot slot) {
            super(slot.methodName, ScopeKind.TYPE);
            this.wrapperSpec = new WrapperSpecification(slot);
        }

        @Override
        Object asAttribute(PyType objclass, Lookup lookup)
                throws InterpreterError {
            return wrapperSpec.createDescr(objclass, lookup);
        }

        @Override
        void add(Method method) {
            super.add(method);
            wrapperSpec.add(method);
        }
