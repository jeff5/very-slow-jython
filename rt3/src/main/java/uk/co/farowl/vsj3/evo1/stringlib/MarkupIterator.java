 ...
 * returning successive 4-tuples, the sequence being equivalent to the
 * original string.
 */
public class MarkupIterator implements CraftedPyObject {
        ...
        try {
            ...
            if (chunk != null) {
                ...
                if (chunk.fieldName == null) {
                    // A field name is null only if there was no
                    // replacement field at
                    // all.
                    ...
                    /*
                     * There may have been a conversion specifier (if
                     * not, None is signalled).
                     */
    ...
    Chunk nextChunk() {
        ...
        /*
         * pos = index is the index of the first text not already
         * chunked.
         */
        int pos = index;

        /*
         * Advance pos to the first '{' that is not a "{{" (escaped
         * brace), or pos<0 if none such.
         */
        while (true) {
        ...
        if (pos < 0) {
            /*
             * ... except pos<0, and there is no further format
             * specifier, only literal text.
             */
            result.literalText =
                    unescapeBraces(markup.substring(index));
            index = markup.length();

        } else {
            // Grab the literal text, dealing with escaped braces.
            result.literalText =
                    unescapeBraces(markup.substring(index, pos));
            /*
             * Scan through the contents of the format spec, between the
             * braces. Skip one '{'.
             */
            pos++;
            int fieldStart = pos;
            ...
            while (pos < markup.length()) {
                if (markup.charAt(pos) == '{') {
                    /*
                     * This means the spec we are gathering itself
                     * contains nested specifiers.
                     */
                    count++;
                    result.formatSpecNeedsExpanding = true;
                } else if (markup.charAt(pos) == '}') {
                    /*
                     * And here is a '}' matching one we already
                     * counted.
                     */
                    count--;
                    if (count == 0) {
                        /*
                         * ... matching the one we began with: parse the
                         * replacement field.
                         */
                        parseField(result,
                                markup.substring(fieldStart, pos));
                        pos++;
                        break;
                    }
                }
    ...
    private void parseField(Chunk result, String fieldMarkup) {
        ...
        if (pos >= 0) {
            /*
             * There's a '!' or a ':', so what precedes the first of
             * them is a field name.
             */
            ...
            if (fieldMarkup.charAt(pos) == '!') {
                ...
                result.conversion =
                        fieldMarkup.substring(pos + 1, pos + 2);
                pos += 2;
                /*
                 * And if that's not the end, there ought to be a ':'
                 * now.
                 */
                if (pos < fieldMarkup.length()) {
                    ...
                    /*
                     * So the format specifier is from the ':' to the
                     * end.
                     */
                ...
                /*
                 * No '!', so the format specifier is from the ':' to
                 * the end. Or empty.
                 */
                result.formatSpec = fieldMarkup.substring(pos + 1);
            }
        ...
        /*
         * Automatic numbers must also work when there is an .attribute
         * or [index].
         */
    ...
    static final class FieldNumbering {
        ...
        /**
         * Remember we are numbering manually, and raise an error if
         * already started numbering automatically.
         */
